...
class Qnvec:
    def __init__(self, n:np.int64, N:np.int64):
        #print("n",n)
        self.vt=np.ndarray(shape=(n),dtype=qnn.Qnnum) # 1D array
        self.shape=n
        self.ndim=1
        self.N=N
...
def zerov(qnzero:qnn.Qnnum,n:np.int64): # qnnumber zero vector
...
def mul_vectors_i(vs:Qnvec, coeff:int):
    if vs.ndim==2:
        ...
        for i in range(2):
            for j in range(n):
                a[i]=vs[i].vt[j]*coeff #mul_vector(v,coeff)
        return a
    else:
        print('incorrect shape')
        return
...
def cross(v1:Qnvec, v2:Qnvec) -> qnn.Qnnum:
    N=v1.N
