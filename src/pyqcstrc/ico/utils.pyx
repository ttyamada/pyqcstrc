...
cpdef np.ndarray remove_doubling_dim3_in_perp_space(np.ndarray[DTYPE_int_t, ndim=3] obj):
    ...
    cdef int i1,num
    cdef np.ndarray[DTYPE_int_t,ndim=1] v4,v5,v6,b
    cdef np.ndarray[DTYPE_int_t,ndim=2] tmp2
    cdef np.ndarray[DTYPE_int_t,ndim=3] tmp3
    #cdef list b
    ...
    num=len(obj)
    if num>1:
        tmp3=np.zeros((num,3,3),dtype=np.int64)
        for i1 in range(num):
            tmp3[i1]=projection3(obj[i1][0],obj[i1][1],obj[i1][2],obj[i1][3],obj[i1][4],obj[i1][5])
        b=np.unique(tmp3,return_index=True,axis=0)[1]
        num=len(b)
        tmp3=np.zeros((num,6,3),dtype=np.int64)
        for i in range(num):
            tmp3[i]=obj[b[i]]
        return tmp3
    else:
