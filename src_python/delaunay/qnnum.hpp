...
namespace qnnum {    
    ...
    public:
        ...
        Qnnum operator+=(const Qnnum& b) {
            ...
            int g = std::gcd(c1, std::gcd(c2, c3));
            if (c3<0) {
                this->n[0]=-c1 / g; this->n[1]=-c2 / g; this->n[2]=-c3 / g;
                return *this;
            } else {
                this->n[0]=c1 / g; this->n[1]= c2 / g; this->n[2]=c3 / g;
                return *this;
        ...
        Qnnum operator*=(const Qnnum& b) {
            ...
            int c2 = a.n[0] * b.n[1] + a.n[1] * b.n[0];
            int c3 = a.n[2] * b.n[2];
            if (c3==0 && c1>0) return *this=Qnnum(1,0,0);  // inf
            if (c3==0 && c1<0) return *this=Qnnum(-1,0,0); // -inf
            if (c3==0 && c1==0) return *this=Qnnum(0,0,0);  // nan
            int g = std::gcd(c1, std::gcd(c2, c3));
            if (c3<0) {
                int d1[] = {-c1 / g, -c2 / g, -c3 / g}; return *this=Qnnum(d1);
            } else {
                int d1[] = {c1 / g, c2 / g, c3 / g};  return *this=Qnnum(d1);
            }
        }
        ...
        Qnnum operator/=(const Qnnum& b) {
            ...
            if (b == Qnnum(0,0,1)) {
                // if this>0 return infinity else this<0 return -infinity
                if (a > Qnnum(0,0,1)) {
                    return *this=Qnnum(1,0,0);  // inf
                } else if (a < Qnnum(0,0,1)) {
                    return *this=Qnnum(-1,0,0); // -inf
                } else if (a.n[2] == 0) {  // inf/inf = nan
                    return *this=Qnnum(0,0,0);  // nan
                }
            }
