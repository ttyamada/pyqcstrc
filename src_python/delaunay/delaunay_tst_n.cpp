...
int main() {
    ...
    //const int n=vt.size();
    int np = 52; // 48 or 51 for triangle

    Qnnum qnv[np][2]; // only for qnnumber print
    for (int i=0; i<np; ++i) {
        for (int j=0; j<2; ++j) {
    ...
    std::vector<point<Qnnum>> points = red_points(qnv, np); 
    np = points.size();
    std::cout<<"np "<<np<<std::endl;
    ...
    vector<triangle<double>> tria=triangulate<double>(pointsf); // Delaynay triangulation
    ...
    eps = 0.000001;
    for (int i=0; i<n; ++i) {
        indx[i][0]=get_indx<double>(tria[i].a.x, tria[i].a.y,pointsf,eps);
        indx[i][1]=get_indx<double>(tria[i].b.x, tria[i].b.y,pointsf,eps);
        indx[i][2]=get_indx<double>(tria[i].c.x, tria[i].c.y,pointsf,eps);
