...
class Qnmat:
    ...
    def set_mt(self:Self,mt:np.array): # QnNdarray*
        ...
        for i in range(shape[0]):
            for j in range(shape[1]):
                self.m[i][j]=qnn.copy(m[i][j]) # copy qnnum
                
    def __copy__(a:Self):
        return copy(a)
    
def qnmat_init(m: NDArray):
    qnm=Qnmat(m)
    return qnm

def zerom(n1: np.int64, n2: np.int64) -> Qnmat:
    shape=(n1,n2)
    qn0=qnn.Qnnum([0,0,1])
    qnm=np.full(shape,qn0)
    return qnm

def zeroms(shape:np.int64) -> qna.QnNdarray:  # qnmatrix array
    nm=shape[0]
    n=shape[1]
...
def anym(m: NDArray[type(qnn.Qnnum)]) -> Qnmat:
    shape=m.shape
    m1=Qnmat(shape)
    for i in range(shape[0]):
        for j in range(shape[1]):
...
def printqnm(str:str,qnm: Qnmat):
    ...
    #                print(qnn.qn2npa(qnm.m[i][j][k]),end=" ")
    #            print("]")
    #    print("")
    else:
        print("ord in printqnm should be 1 or 2  but",ord); exit()
