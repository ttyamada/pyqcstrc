...
class Qnvec:
    ...
    def __init__(self, v):
        raise ValueError("unsupported Qnvec argument : 1D Qnnum or 2D int array required")
    ...
    def _(self, v):
        ...
        self.ndim=v.ndim
        if (ndim!=2):
            raise ValueError("unsupported Qnvec argument, NPArray shoud be 2D")

        for i in range(self.shape[0]):
            for j in range(self.shape[1]):
                self.v[i]=qnn.Qnnum([v[i][0], v[i][1], v[i][2]])

    @__init__.register(qnn.Qnnum)
    def _(self, v):
        #def _(self, v: NDArray[type(qnn.Qnnum)]):
        self.v=v
        self.ndim=v.ndim
        if (ndim!=1):
            raise ValueError("unsupported Qnvec argument : Qnnum array should be 1D")
...
def zerov(n: np.int64)->Qnvec: # qnnumber zero vector with size n
    v = np.zeros((n,3)) # 1D array with size n
    for i in range(n):
        v[i][0]=0
        v[i][1]=0
        v[i][2]=1
