...
import numpy as np
from math import sqrt  # this is needed only for export radius
...
class Delaunay2D:
    ...
    def __init__(self, center, radius):
        ...
        M1=qnn.Qnnum([1,0,1])
        M2=qnn.Qnnum([-1,0,1])
        qnv1=qnv.anyv(np.array([M2,M2]))
        qnv2=qnv.anyv(np.array([M1,M2]))
        qnv3=qnv.anyv(np.array([M1,M1]))
        qnv4=qnv.anyv(np.array([M2,M1]))
        #self.coords = [center+radius*np.array((-1, -1)),
        #               center+radius*np.array((+1, -1)),
        #               center+radius*np.array((+1, +1)),
        #               center+radius*np.array((-1, +1))]
        self.coords = np.array([center+qnv1,
                                center+qnv2,
                                center+qnv3,
                                center+qnv4])
    ...
    def circumcenter(self, tri):
        ...
        qnn.printqnn("qn2",qn2)
        print("pts.shape",pts.shape)
        print("pts2.shape",pts2.shape)
        qnm.printqnm("pts",pts)
        qnm.printqnm("pts2",pts2)
        pts2s=pts2*qn2
        qnm.printqnm("pts2s",pts2s)
        # add 1 for last line and column but [4][4] element is zero
        A=np.bmat([[pts2 * qn2, [[qn1],
                                [qn1],
                                [qn1]]],
                    [[[qn1,qn1,qn1,qn0]]]])
        #A = np.bmat([[2 * pts2, [[1],
        ...
        b = np.hstack((np.sum(pts * pts, axis=1), [qn1]))
        print("b.shape",b.shape)
        qnv.printqnv("b",b)

        # Qnnum type solve should be added
        # linalg.solve uses lapack gesv
