...
#include "qnnum.hpp"  // newest one under ../delaunay/
//#include "qnvec.hpp" // this should be replaced by geometry.hpp
#include "qnmat.hpp"
#include "numeric.hpp" // under ../delaunay/
#include "geometry_n"  // for point, edge, circle, triangle under ../delaunay/
...
edge[] get_edge(const triangle& tri) {
    // Create a 3 x 2 point array.
    edges= edge[3];  // array of edge for three edges of a triangle
    edges[0]=edge(tri.a, tri.b);
    edges[1]=edge(tri.b, tri.c);
    edges[3]=edge(tri.a, tri.c);
    return edges;
}

// Function: det_vecabc
qnnum::Qnnum det_vecabc(const point& a, const point& b, const point& c) {
    point da = a-c;  // overloading of - for point is assumed
    point db = b-c;
    return da.x * db.y - db.x * da.y;
}

// Function: counter_clockwise
triangle counter_clockwise(triangle& tri) {
    if (det_vecabc(tri.a, tri.b, tri.c) > qnnum::zero()) {
        return tri;
    } else { // swap tri[0] and tri[1]
        point tmp(tri.a.x, tri.a.y);
        tri.a = copy(tri.b);
        tri.b = copy(tmp);
        return tri;
    }
