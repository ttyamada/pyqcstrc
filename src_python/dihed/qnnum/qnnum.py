...
# define qnnumber  (n1+n2\sqrt(N))/n3
# N=2 5 3 for octagonal, decagonal, dodecagonal QCs
# which is defined in crsys
# Qnnum([n1,n2,n3]) generate qnnumber represented by (n1+n2\sqrt(N))/n3
class Qnnum:
...
def qnnum_init(n: NDArray[np.int64]):
    return Qnnum(n)
...
def any_i(n_: NDArray[np.int64]) -> Qnnum:
    return Qnnum([n_,0,1])

def any(n_:NDArray[np.int64]) -> Qnnum:
    return Qnnum([n_[0],n_[1],n_[2]])

def add(a:Qnnum, b:Qnnum) -> Qnnum:
...
def iadd(self:Qnnum, b:Qnnum) -> Qnnum:
    self=add(self,b)
    return self

def sub(a:Qnnum, b:Qnnum) -> Qnnum:
    c1=a.n[0]*b.n[2]-b.n[0]*a.n[2]
    c2=a.n[1]*b.n[2]-b.n[1]*a.n[2]
    c3=a.n[2]*b.n[2]
    x=np.array([c1,c2,c3],dtype=np.int64)
...
def isub(self:Qnnum, b:Qnnum) -> Qnnum:
    self=sub(self,b)
    return self
...
def mul_i(a:Qnnum, b:np.int64) -> Qnnum: # b should be int
    c1=a.n[0]*b
    c2=a.n[1]*b
    c3=a.n[2]
    x=np.array([c1,c2,c3],dtype=np.int64)
...
def div_i(a:Qnnum, b:np.int64) -> Qnnum: # b should be int
    c1=a.n[0]
    c2=a.n[1]
    c3=a.n[2]*b
    c=Qnnum(np.array([c1,c2,c3]))
...
def pow(a:Qnnum, b:np.int64) -> Qnnum:
    if b==0:
        return Qnnum([1,1,1])
    elif b>0:
        c=one()
