...
def slope(a, b):
    #return std if a.x - b.x == 0.0 else : math.inf : (a.y - b.y) / (a.x - b.x)
    if a.x - b.x == 0.0:
        return math.inf
    else :
...
def valid():
    ...
    return abs(area) > sys.float_info.epsilon
...
def circumcircle():
    ...
    if not valid():
        return circle(point(0, 0), sys.float_info.epsilon)
