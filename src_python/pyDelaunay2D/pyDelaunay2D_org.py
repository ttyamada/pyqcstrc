...
class Edge(np.ndarray):
	...
	def isEqual(self, other_edge:Self):
		if (self[0].isEqual(other_edge[0]) or self[1].isEqual(other_edge[1])) and \
...
class Graph(np.ndarray):
	def __init__(self):
		...
		#self._points = []
		self._points = np.zeros((0),dtype=Point)
		
		#This will be a list of triangle objects as defined above
		#self._triangles = []
		self._triangles = np.zeros((0,3),dtype=Point)
		
		#This is a list of edges as defined above
		#self._edges = []
		self._edges = np.zeros((0,2),dtype=Point)
	...
	def addPoint(self, point:Point):
		...
		if self._point_min_x > point.pos()[0] or self._point_min_x == 0:
			self._point=np.concatenate(self._point,point)
		...
		#If the X value is somewhere in the middle
			...
			# compare the Y values to find which order they go in
				...
				if same_x[0].pos()[1] > point.pos()[1]:
					self._points=insert(self._points, index - 1, point)
					return True
				else:
					self._points=insert(self._points, index + 1, point)
					return True
			...
			#the new point needs to go based on its Y value
				...
				if(first_greater_y != 0):
					self._points=insert(self._points,first_greater_y, point)
					return True
				else:
					self._points=insert(self._points,self._points.index(same_x[len(same_x) - 1]), point)
					return True
	...
	def addTriangle(self, triangle:Triangle):
		...
		tri = [ triangle[0].pos(), triangle[1].pos(), triangle[2].pos() ]
		return True
...
def pointInCircle(point:Point, circle:NDArray[Point]):
