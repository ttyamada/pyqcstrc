...
cpdef np.ndarray outer_product(np.ndarray[DTYPE_int_t, ndim=2] v1,np.ndarray[DTYPE_int_t, ndim=2] v2):
    ...
    #return tmp2
    return np.array([[c1,c2,c3],[c4,c5,c6],[c7,c8,c9]])
...
cpdef np.ndarray add_vectors(np.ndarray[DTYPE_int_t, ndim=2] vec1,
    ...
    cdef DTYPE_int_t i1,n1,n2,n3
    cdef list a
    a=[]
    for i1 in range(6):
        n1,n2,n3=add(vec1[i1][0],vec1[i1][1],vec1[i1][2],vec2[i1][0],vec2[i1][1],vec2[i1][2])
        a.append([n1,n2,n3])
