...
from pyqcstrc.ico.math1 cimport centroid, coplanar_check, projection,projection3, det_matrix, dot_product, add, sub, mul, div
from pyqcstrc.ico.numericalc cimport obj_volume_6d_numerical, tetrahedron_volume_6d_numerical, inside_outside_tetrahedron, check_intersection_segment_surface_numerical
from pyqcstrc.ico.utils cimport obj_volume_6d, tetrahedron_volume_6d, remove_doubling_dim3_in_perp_space, remove_doubling_dim4_in_perp_space, generator_edge, generator_surface_1
...
cpdef list intersection_two_obj_convex(np.ndarray[DTYPE_int_t, ndim=4] obj1,
    ...
    obj1_surf=generator_surface_1(obj1,verbose)
    obj2_surf=generator_surface_1(obj2,verbose)
    obj1_edge=generator_edge(obj1_surf,verbose)
    obj2_edge=generator_edge(obj2_surf,verbose)
    ...
    for i1 in range(len(obj1_surf)):
        ...
        for i2 in range(len(obj2_edge)):
            ed2=obj2_edge[i2]
            tmp=intersection_line_segment_triangle(ed2,tr1,verbose-1)
            if len(tmp)!=1:
                if counter==0:
                    p=tmp
                else:
    ...
    for i1 in range(len(obj2_surf)):
        ...
        for i2 in range(len(obj1_edge)):
            ed1=obj1_edge[i2]
            tmp=intersection_line_segment_triangle(ed1,tr2,verbose-1)
            if len(tmp)!=1:
                if counter==0:
                    p=tmp
                else:
            ...
            if coplanar_check(tmp3a)==0:
                if len(tmp3a)==4:
                    ...
                    if verbose>0:
                        ...
                        print('              volume (numerical value) %8.6f'%(tetrahedron_volume_6d_numerical(tmp4a[0])))
                    else:
                        pass
                else:
...
@cython.boundscheck(False)
@cython.wraparound(False)
cpdef np.ndarray intersection_two_obj_1(np.ndarray[DTYPE_int_t, ndim=4] obj1,
                                np.ndarray[DTYPE_int_t, ndim=4] obj2,
                                int fflag,
                                int verbose):
    cdef int falg,i1,i2,counter,counter1
    cdef long a0,b0,c0,a1,b1,c1
    cdef double dd2
    cdef np.ndarray[DTYPE_int_t,ndim=2] cent2
    cdef np.ndarray[DTYPE_int_t,ndim=4] common4,tmp4,tmp_common4
    
    if fflag==0: # filtering ON, rough check intersection of tetrahedron in obj1 and obj2
        cent2=centroid_obj(obj2)
        dd2=ball_radius_obj(obj2,cent2)
    else:
        pass
    ...
    if fflag==0: # filter ON
        for i1 in range(len(obj1)):
            ...
            if rough_check_intersection_tetrahedron_obj(obj1[i1],cent2,dd2)==0:
                ...
                a0,b0,c0=tetrahedron_volume_6d(obj1[i1])
                for i2 in range(len(obj2)):
                    flag=check_intersection_two_tetrahedron_4(obj1[i1],obj2[i2],verbose-1)
                    if flag==3:# tetrahedron_1 and tetrahedron_2 are intersecting
                        tmp4=intersection_two_tetrahedron_4(obj1[i1],obj2[i2],verbose)
                        if tmp4.tolist()!=[[[[0]]]]:
                            if counter1==0:
                                tmp_common4=tmp4
                                counter1+=1
                            ...
                            pass
                    elif flag==1:# tetrahedron_1 is inside tetrahedron_2
                        if counter==0:
                            common4=obj1[i1].reshape(1,4,6,3)
                            counter+=1
                        else:
                            common4=np.vstack((common4,[obj1[i1]]))
                    elif flag==2:# tetrahedron_2 is inside tetrahedron_1
                        #if counter==0:
                            #common4=obj2[i2].reshape(1,4,6,3)
                            #counter+=1
                        if counter1==0:
                            tmp_common4=obj2[i2].reshape(1,4,6,3)
                            counter1+=1
                        else:
                            #common4=np.vstack((common4,[obj2[i2]]))
                            tmp_common4=np.vstack((tmp_common4,[obj2[i2]]))
                    else:
                        pass
                if counter1>0:
                    a1,b1,c1=obj_volume_6d(tmp_common4)
                    if a1==a0 and b1==b0 and c1==c0:
                        if counter==0:
                            common4=obj1[i1].reshape(1,4,6,3)
                            counter+=1
                        else:
                            common4=np.vstack((common4,obj1[i1].reshape(1,4,6,3)))
                    else:
                        if counter==0:
                            common4=tmp_common4
                            counter+=1
...
@cython.boundscheck(False)
@cython.wraparound(False)
cpdef np.ndarray intersection_two_obj_2(np.ndarray[DTYPE_int_t, ndim=4] obj1,
                                        np.ndarray[DTYPE_int_t, ndim=4] obj2,
                                        int verbose):
    # modified version of intersection_two_obj_1() for simplification using convex hull
    
    # ob1: convex hull of obj2
    # ob2: obj that to be simplified
    
    cdef int falg,i1,i2,counter,counter1,counter2
    cdef long a0,b0,c0,a1,b1,c1
    cdef np.ndarray[DTYPE_int_t,ndim=4] common4,common4_tmp,tmp4,tmp_common4
    cdef list lst
    
    common4=np.array([[[[0]]]])
    lst=[]
    counter=0  # counter for common4
    counter2=0 # counter for common4_tmp
    for i1 in range(len(obj1)):
        if verbose>0:
            print('      %d-th tetrahedron in obj1'%(i1))
        else:
            pass
        counter1=0 # counter for tmp_common4
        #tmp_common4=np.array([[[[0]]]])
        a0,b0,c0=tetrahedron_volume_6d(obj1[i1])
        for i2 in range(len(obj2)):
            flag=check_intersection_two_tetrahedron_4(obj1[i1],obj2[i2],verbose-1)
            if flag==3:# tetrahedron_1 and tetrahedron_2 are intersecting
                ...
                if tmp4.tolist()!=[[[[0]]]]:
                    ...
                    #    counter2+=1
                    #else:
                    #    common4_tmp=np.vstack((common4_tmp,tmp4))
                    if counter1==0:
                        tmp_common4=tmp4
                        counter1+=1
                    else:
                ...
                if counter2==0:
                    common4_tmp=obj1[i1].reshape(1,4,6,3)
                    counter2+=1
                else:
                    common4_tmp=np.vstack((common4_tmp,[obj1[i1]]))
            elif flag==2:# tetrahedron_2 is inside tetrahedron_1
                if counter1==0:
                    tmp_common4=obj2[i2].reshape(1,4,6,3)
                    counter1+=1
        ...
        if counter1>0:
            ...
            if a1==a0 and b1==b0 and c1==c0:
                ...
                if counter2==0:
                    common4_tmp=tmp_common4
                    counter2+=1
                else:
                    common4_tmp=np.vstack((common4_tmp,tmp_common4))
        else:
            pass
    
    if counter2>0:
        # intersection obj2 and common4_tmp
        for i1 in range(len(obj2)):
            counter1=0
            a0,b0,c0=tetrahedron_volume_6d(obj2[i1])
            for i2 in range(len(common4_tmp)):
                flag=check_intersection_two_tetrahedron_4(obj2[i1],common4_tmp[i2],verbose-1)
                if flag==3:# tetrahedron_2 and common4_tmp[i2] are intersecting
                    tmp4=intersection_two_tetrahedron_4(obj2[i1],common4_tmp[i2],verbose)
                    if tmp4.tolist()!=[[[[0]]]]:
                        if counter1==0:
                            tmp_common4=tmp4
                            counter1+=1
                        ...
                        pass
                elif flag==1:# tetrahedron_2 is inside common4_tmp[i2]
                    if counter==0:
                        common4=obj2[i1].reshape(1,4,6,3)
                        counter+=1
                    else:
                        common4=np.vstack((common4,[obj2[i1]]))
                elif flag==2:# common4_tmp[i2] is inside tetrahedron_2
                    if counter1==0:
                        tmp_common4=common4_tmp[i2].reshape(1,4,6,3)
                        counter1+=1
                    else:
                        tmp_common4=np.vstack((tmp_common4,[common4_tmp[i2]]))
                else:
                    pass
            if counter1>0:
                a1,b1,c1=obj_volume_6d(tmp_common4)
...
@cython.boundscheck(False)
@cython.wraparound(False)
cpdef np.ndarray intersection_two_obj_3(np.ndarray[DTYPE_int_t, ndim=4] obj1,
                                        np.ndarray[DTYPE_int_t, ndim=4] obj2,
                                        int verbose):
    # modified version of intersection_two_obj_1() for simplification using convex hull
    
    # ob1: obj that to be simplified
    # ob2: convex hull of obj
...
@cython.boundscheck(False)
@cython.wraparound(False)
cdef int check_intersection_two_tetrahedron_4(np.ndarray[DTYPE_int_t, ndim=3] tetrahedron_1,
                                                np.ndarray[DTYPE_int_t, ndim=3] tetrahedron_2,
                                                int verbose):
    cdef int i1
    ...
    if verbose>0:
        print('        check_intersection_two_tetrahedron_4()')
    else:
        pass
    #
    # -----------------
    ...
    counter1=0
    for i1 in range(24): # len(combination_index) = 24
        # case 1: intersection between
        # 6 edges of tetrahedron_1
        # 4 surfaces of tetrahedron_2
        segment_1=tetrahedron_1[comb[i1][0]] 
        ...
        surface_2=tetrahedron_2[comb[i1][3]]
        surface_3=tetrahedron_2[comb[i1][4]]
        if check_intersection_segment_surface_numerical(segment_1,segment_2,surface_1,surface_2,surface_3)==0: # intersectiing
            counter1+=1
            break
        else:
            pass
    ...
    # get vertces of tetrahedron_1 which are inside tetrahedron_2
    for i1 in range(4):
        if inside_outside_tetrahedron(tetrahedron_1[i1],tetrahedron_2[0],tetrahedron_2[1],tetrahedron_2[2],tetrahedron_2[3])==0: # inside
            pass
        else:
            counter2+=1
            break
    ...
    # get vertces of tetrahedron_2 which are inside tetrahedron_1
    for i1 in range(4):
        if inside_outside_tetrahedron(tetrahedron_2[i1],tetrahedron_1[0],tetrahedron_1[1],tetrahedron_1[2],tetrahedron_1[3])==0: # inside
            pass
        else:
            counter3+=1
    
    if counter2==0 :
        return 1 # tetrahedron_1 is inside tetrahedron_2
    elif counter3==0:
        return 2 # tetrahedron_2 is inside tetrahedron_1
    else:
        if counter1>0:
            return 3 # intersecting
        else:
            return 0 # no intersecting
...
cdef np.ndarray intersection_line_segment_triangle(np.ndarray[DTYPE_int_t, ndim=3] line_segment,
                                                    ...
                                                    int verbose):
    return intersection_segment_surface(line_segment[0],line_segment[1],triangle[0],triangle[1],triangle[2],verbose)
    #cdef int i,j,counter,num1,num2,num3
    #cdef np.ndarray[DTYPE_int_t,ndim=1] p
    #cdef np.ndarray[DTYPE_int_t,ndim=1] tmp,tmp1
    #
    # -----------------
    # triangle
    # -----------------
...
#def intersection_segment_surface(np.ndarray[DTYPE_int_t, ndim=2] segment_1,np.ndarray[DTYPE_int_t, ndim=2] segment_2,np.ndarray[DTYPE_int_t, ndim=2] surface_1,np.ndarray[DTYPE_int_t, ndim=2] surface_2,np.ndarray[DTYPE_int_t, ndim=2] surface_3):
    ...
    seg1i1,seg1i2,seg1i3=projection3(segment_1[0],segment_1[1],segment_1[2],segment_1[3],segment_1[4],segment_1[5])
    seg2i1,seg2i2,seg2i3=projection3(segment_2[0],segment_2[1],segment_2[2],segment_2[3],segment_2[4],segment_2[5])
    sur1i1,sur1i2,sur1i3=projection3(surface_1[0],surface_1[1],surface_1[2],surface_1[3],surface_1[4],surface_1[5])
    sur2i1,sur2i2,sur2i3=projection3(surface_2[0],surface_2[1],surface_2[2],surface_2[3],surface_2[4],surface_2[5])
    sur3i1,sur3i2,sur3i3=projection3(surface_3[0],surface_3[1],surface_3[2],surface_3[3],surface_3[4],surface_3[5])
...
@cython.boundscheck(False)
@cython.wraparound(False)
cpdef double distance_in_perp_space(np.ndarray[DTYPE_int_t, ndim=2] pos1,
