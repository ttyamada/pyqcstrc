...
def gen_reference(nmax,size,verbose=0):
    lst=[]
...
if __name__ == "__main__":
    ...
    ref=gen_reference(nmax,size,verbose)
    print('done')
    ...
    for i1 in range(2,len(a)):
        ...
        #
        lst.append([site,atom,spnvec])
    print('done')
    
    print('comparison')
    ofname='%s_nmax%d'%(basename,nmax)
    f=open('%s'%('%s/%s.out'%(wpath,ofname)),'w')
    counter=0
    out=[]
    for a in lst: # data
        xyz_data=a[0]
        ...
        for b in ref:
            ...
            if np.allclose(xyz_data, xyz_ref, rtol=1e-03, atol=1e-06):
                ...
                out.append([element,-vi,counter+1,b[2],spnvec,0])
                counter+=1
                break
    f.close()
    print('done')
    ...
    od.write_vesta(out,path=wpath,basename=ofname,color='k',select='mag',verbose=0)
    print('done')
